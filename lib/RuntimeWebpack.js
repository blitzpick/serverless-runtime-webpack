module.exports = function (S) {
    const SError = require(S.getServerlessPath("Error")),
        ...
        fs = require("fs-extra"),
        PackExternalModules = require("./PackExternalModules"),
        PackageModules = require("./packageModules");
     ...
     */
        ...
        scaffold(func) {
            ...
            return Promise.all([
                fs.readFile(handlerPath),
                fs.readFile(webpackPath)
            ])
        ...
        run(func, stage, region, event) {
            return this
                ...
                .then((locals) => {
                    ...
                    if (child.error) return Promise.reject(child.error);
     ...
     */
        ...
        build(func, stage, region) {
            // Validate
            if (!func._class || func._class !== "Function") return Promise.reject(new SError("A function instance is required"));
            ...
            return this.createDistDir(func.name).then((pathDist) => {
                return this.copyFunction(func, pathDist, stage, region)
                    ...
                    .then((compileStats) => { stats = compileStats; })
                    .then(() => PackExternalModules.packExternalModules(func, stats, pathDist))
                    .then(() => PackageModules.packageModules(func, stats, pathDist))
                    .then(() => pathDist);
            });
        }
            ...
            return PromiseHelper
                ...
                .then((stats) => {
                    ...
                    _.forEach(stats.stats, (compileStats) => {
                        SCli.log(compileStats.toString(consoleStats));
                    ...
                    // Workaround for importing stuff from serverless-webpack
                    return { stats: [stats] };
                });
        }
        ...
        //         .reject("built")
        //         .map(module => /external "(.+)"/.exec(module.identifier))
        //         .map(module => _.last(module))
        ...
        //         .then(modulePaths => _(modulePaths).flattenDeep().compact().uniqBy()
        //             .value())
        //         .then(modulePaths => this._copyModules(modulePaths, destFolder));
        // }

        // _copyModules(modulePaths, destFolder) {
        //     console.log("_copyModules");

        //     return Promise.all(_.map(modulePaths, (modulePath) => {
        //         const moduleName = path.basename(modulePath);
        //         const destPath = path.join(destFolder, moduleName);

        //         return fs.copy(modulePath, destPath);
        //     }));
        // }

        // _getAllModulePaths(rootPath, moduleNames) {
        //     // console.log("_getAllModulePaths");
        //     return Promise.all(_.map(moduleNames, moduleName => this._getModulePaths(rootPath, moduleName)));
        // }

        // _getModulePaths(rootPath, moduleName) {
        //     // console.log(JSON.stringify({name: "_getModulePaths", moduleName}));
        ...
        //     return fs.readFile(`${modulePath}/package.json`, "utf-8")
        //         .then((pkgJson) => {
        //             try {
     ...
     */
        ...
        _addEnvVarsInline(func, pathDist, stage, region) {
          ...
          `;
                    ...
                    return fs.writeFile(path.join(pathDist, handlerDir, "_serverless_handler.js"), loader);
                });
        }
    }
