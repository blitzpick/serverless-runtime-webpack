...
module.exports = {
  ...
  packExternalModules(func, stats, pathDist) {
    ...
    return PromiseHelper.fromCallback(cb => {
    ...
    .then(dependencyGraph => {
      ...
      const compositeModulePath = path.join(pathDist, 'dependencies');
      ...
      return PromiseHelper.fromCallback(cb => fse.pathExists(packageLockPath, cb))
      .then(exists => {
        if (exists) {
          console.log('Package lock found - Using locked versions');
          try {
            const packageLockJson = fs.readFileSync(packageLockPath);
            /**
            ...
            fs.writeFileSync(path.join(compositeModulePath, 'package-lock.json'), JSON.stringify(packageLockJson, null, 2));
          } catch(err) {
      ...
      .mapSeries(compileStats => {
        ...
        fs.writeFileSync(modulePackageJson, JSON.stringify(modulePackage, null, 2));

        const startCopy = _.now();
        return PromiseHelper.fromCallback(callback => fse.copy(path.join(compositeModulePath, 'node_modules'), path.join(modulePath, 'node_modules'), callback))
        .tap(() => console.log(`Copy modules: ${modulePath} [${_.now() - startCopy} ms]`))
        .then(() => {
          // Prune extraneous packages - removes not needed ones
          const startPrune = _.now();
          return PromiseHelper.fromCallback(callback => {
            childProcess.exec('npm prune', {
              cwd: modulePath
            }, callback);
          })
          .tap(() => console.log(`Prune: ${modulePath} [${_.now() - startPrune} ms]`));
        });
      })
      .return();
    });
