    ...
    if (moduleVersion) {
      ...
      try {
        ...
        if (!_.isEmpty(peerDependencies)) {
          ...
          const peerModules = getProdModules.call(this, _.map(peerDependencies, (value, key) => ({
            external: key
...
module.exports = {
  ...
  packExternalModules(func, stats, pathDist) {
    ...
    const ignoredNpmErrors = [
      ...
      {
        npmError: "peer dep missing",
        log: true
      }
    ];

    return (new Promise((resolve, reject) => childProcess.exec(command, {
      ...
      .then((dependencyGraph) => {
        ...
        return fse.pathExists(packageLockPath)
          ...
          .then(() => {
            ...
            return (new Promise((resolve, reject) => childProcess.exec("yarn", {
